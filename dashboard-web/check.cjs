...
(async()=>{
 ...
 try{
  ...
  assert.deepEqual(await page.locator('#season option').allTextContents(),['2025','2024','2023','2022','2021','2020','2019']);
  await page.locator('#season').selectOption('2019');
  await page.locator('.hero').waitFor();
  assert((await page.locator('.hero').innerText()).includes('2019 SEASON SPOTLIGHT'));
  await page.locator('#season').selectOption('2025');
  ...
  const currentSeason=Number(await page.locator('#season').inputValue());
  const top=data.player_passing_leaders.filter(r=>Number(r.season)===currentSeason).sort((a,b)=>b.passing_yards-a.passing_yards)[0];
